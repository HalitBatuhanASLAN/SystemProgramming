 ...
 *   - allocates them with mmap(MAP_SHARED|MAP_ANONYMOUS) so every child
 *     forked AFTER allocation sees the exact same bytes;
 *   - initializes the embedded mutexes/conds with PTHREAD_PROCESS_SHARED
 *     attributes so they are valid across process boundaries;
 *   - provides classic bounded-buffer push/pop operations protected by
 *     those primitives, using condition variables to block when full or
 *     empty.
 * ============================================================================= */
...
/* =============================================================================
 * Size helpers
 * The structs end with a flexible array member, so total size is
 * sizeof(struct) + capacity * sizeof(log_entry_t). mmap() needs the exact
 * byte count, so we wrap it here once.
 * ============================================================================= */
 ...
 * Without PTHREAD_PROCESS_SHARED, behavior is undefined when a forked
 * child accesses the mutex stored in shared memory.
 * ============================================================================= */
 ...
 * MAP_ANONYMOUS gives memory not backed by any file. MAP_SHARED tells
 * the kernel that any forked child should see the same physical pages.
...
shm_region_c_t* shm_create_region_c(void)
{
    ...
    /* sem_init's second argument is "pshared": 1 means the semaphore
     * lives in shared memory and is usable across processes. Initial
     * count 0 means sem_wait blocks until a sem_post happens. */
...
void shm_a_push(shm_region_a_t* r, const log_entry_t* e)
{
     ...
     * Only the Reader's parser thread pushes EOFs (one per level per
     * Reader). eof_count_per_level[i] reaches total_readers once every
     * Reader has announced EOF for level i. */
 ...
 *   - otherwise, if every Reader has finished (all_eof_done), return 0
 *     so the Dispatcher knows to exit;
 *   - otherwise, block on pthread_cond_timedwait(not_empty_a). When the
 *     timeout fires, re-check the predicates and either return or wait
 *     again. Spurious wakeups are absorbed by the outer while(1) loop. */
int shm_a_pop_timed(shm_region_a_t* r, log_entry_t* e, int timeout_sec)
{
    ...
    while (1)
    {
        ...
        if (all_eof_done(r))
        {
             ...
             * Returning 0 is the agreed signal to the Dispatcher
             * that no more input will ever arrive. */
            pthread_mutex_unlock(&r->input_mutex);
            return 0;
        }
        ...
        if (rc == ETIMEDOUT)
        {
            /* Re-check predicates after timeout. If buffer is still
             * empty AND every Reader is done, we exit; otherwise we
             * wait again. */
...
/* shm_b_pop: blocking pop. Returns 0 only when the buffer is empty AND
 * the Dispatcher has set eof_posted. Multiple workers may race on the
 * same Region B; the mutex serializes them so each entry is consumed
 * exactly once. */
...
/* shm_d_pop: returns 0 when buffer empty AND the Dispatcher has signaled
 * dispatcher_done. The Aggregator's drain thread loops on this. */
