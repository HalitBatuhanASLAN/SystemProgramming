 ...
 *   2. Drains Region D in parallel (a dedicated thread) to compute HIGH_PRIORITY_SCORE.
 *   3. Sorts level results by total_weighted_score DESC.
 *   4. Writes a human-readable .txt file (right-aligned columns, two-space separators, one decimal place) and a binary .bin file (with a magic number 0xC5E3440B) atomically (write to .tmp then rename).*/
