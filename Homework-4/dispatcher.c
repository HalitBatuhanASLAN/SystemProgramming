 ...
 *
 *   while true:
 *       entry = shm_a_pop_timed(region_a, timeout)
 *       if entry == END_OF_STREAM:                     # all readers done
 *           break
 *       if entry.is_eof:
 *           eof_received[entry.level]++
 *           if eof_received[entry.level] == n_readers:
 *               signal_eof_to_b(region_b[entry.level])
 *           continue
 *       shm_b_push(region_b[entry.level], entry)
 *       if entry.source in priority_sources:
 *           shm_d_push(region_d, entry)
 *
 *   for each level not yet eof-forwarded:
 *       signal_eof_to_b(region_b[level])             # fail-safe
 *   region_d.dispatcher_done = 1
 *
 * Note that EOF markers do NOT cross Region B as data: the Dispatcher
 * counts them in Region A and converts them into a single eof_posted=1
 * flag on each Region B[level]. Workers see "buffer empty AND eof_posted"
 * as the signal to exit cleanly.
 * ============================================================================= */
 ...
 * Returns 1 if the entry's [SOURCE] tag appears in the filter file,
 * 0 otherwise. The list is small (< 100), so linear search is fine.
 * ============================================================================= */
 ...
 * The Dispatcher is the only producer for Region B, so writing eof_posted
 * here is safe under the level_mutex. broadcast() wakes every blocked
 * worker so they all observe eof_posted at once.
 * ============================================================================= */
...
void dispatcher_process_main(dispatcher_arg_t* a)
{
    ...
    /* -------------------------------------------------------------------
     * Main routing loop
     * Pops entries from Region A (timed wait). When shm_a_pop_timed
     * returns 0, every Reader has finished AND Region A is empty;
    ...
    while (1)
    {
        ...
        /* EOF marker handling: count them, forward once we've seen all
         * Readers for this level. */
        if (entry.is_eof)
        {
            int lvl = entry.level;
     ...
     * If for some reason a level never received its expected EOF count
     * (e.g. an empty file produced no entries but still sent EOFs that
     * we did not all see), forward EOF anyway so workers don't hang.
     * ------------------------------------------------------------------- */
    ...
    /* Tell Region D that no more pushes will happen, so the
     * Aggregator's drain thread can return out of shm_d_pop. */
