 ...
 *
 *   1. Spawn drain_region_d() thread to compute HIGH_PRIORITY_SCORE
 *      from Region D in parallel.
 *
 ...
 *      Both have a T_TIMEOUT-second deadline; on timeout we just print
 *      a warning and continue.
 *
 *   3. Join the Region D drain thread to obtain HIGH_PRIORITY_SCORE.
 *
 *   4. Copy the four level_result_t into a local array, sort DESC by
 *      total_weighted_score, then call write_text() and write_binary().
 *
 * Output formatting follows the assignment rules:
 *   - .txt: right-aligned columns, two-space separators, .1f doubles
 *   - .bin: header uint32 magic 0xC5E3440B + version + counts + scores,
 *           followed by raw level_result_t structs; written to a .tmp
 *           file first, then atomically renamed.
 * ============================================================================= */
 ...
 * Little-endian everywhere (the only supported architecture for this
 * assignment is x86_64). Keep the layout in sync with .bin readers and
 * the tests' Python parser.
 * ============================================================================= */
typedef struct
{
...
/* =============================================================================
 * drain_region_d - thread that consumes Region D and accumulates
 *                  HIGH_PRIORITY_SCORE.
 * Mirrors the Analyzer's keyword counting, but runs inside the Aggregator
 * because Region D is independent of the per-level pipelines.
 * ============================================================================= */
typedef struct
{
...
static void write_text(const char* path, level_result_t* res, int n,
                       ...
                       int n_workers, const char* filter_path)
{
    FILE* f = fopen(path, "w");
    if (!f)
    {
    ...
    for (int i = 0; i < n; i++)
    {
        fprintf(f, "%7s  %7ld  %14.1f",
                res[i].level, res[i].total_entries,
                res[i].total_weighted_score);
        for (int k = 0; k < nk; k++)
        {
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int t = 0; t < 3; t++)
        {
            ...
            {
                fprintf(f, "  %s:%ld", res[i].top_source[t],
                        res[i].top_source_hits[t]);
                any = 1;
            }
        }
        if (!any)
    ...
    /* --- Per-thread block. We always emit slots 0..n_workers-1, even
     *     if the value is zero (the assignment expects a fixed shape). */
 ...
 * rename(2) is atomic on POSIX, so a partial write can never appear at
 * "<path>"; if the program crashes mid-write, the .tmp file is leftover
 * but the previous .bin (if any) is intact.
 * ============================================================================= */
...
void aggregator_process_main(aggregator_arg_t* a)
{
    ...
    for (int i = 0; i < LEVEL_COUNT; i++)
    {
        ...
        {
            int rc = pthread_cond_timedwait(&a->region_c->result_cond,
                                            &a->region_c->result_mutex,
                                            &deadline);
            if (rc == ETIMEDOUT)
            {
        ...
        /* (2) sem_timedwait - extra signal that the matching Analyzer
         *     used to mark its result ready. Posted exactly once per
         *     level by the reporting thread, so a second wait would
         *     hang; we use a separate timeout. */
        struct timespec ts2;
    ...
    /* --- Wait for the Region D drain thread to finish. The drain
     *     thread blocks in shm_d_pop until the Dispatcher set
     *     dispatcher_done, which happens when the Dispatcher exits. */
    ...
    write_text  (a->output_path, sorted, LEVEL_COUNT,
                 a->keywords, a->n_keywords, a->n_files,
                 total_w, darg.hp_score,
                 a->n_workers, a->filter_path);
