 ...
 *
 * Each Reader process is forked by main, given one log file path and a thread count T. Inside the Reader process there are:
 *
 *   - T "reader threads" that each read a byte-range chunk of the log file, parse the lines they see, and push the parsed entries into an internal_buf_t (a small in-process ring buffer).
 *
 *   - 1 "parser thread" that drains the internal buffer and forwards every entry into Region A (the cross-process shared buffer). When the internal buffer is exhausted and all reader threads are done, the parser thread pushes one EOF marker per log level into Region A, then exits.
 *
 * The split into reader threads + parser thread isolates I/O parallelism from the cross-process synchronization cost.
 *
 * The internal_buf_t uses ONLY pthread_mutex + pthread_cond - no semaphores, in line with the assignment's restriction.*/
...
/*
 * internal_buf_t - in-process ring buffer used between reader threads (producers) and the parser thread (consumer). Lives on the heap of the Reader process; NOT visible to other processes.*/
typedef struct
{
    pthread_mutex_t mu;
...
/* Parses a single log line into log_entry_t. Returns 1 on success, 0 if the line is malformed and should be skipped silently.*/
