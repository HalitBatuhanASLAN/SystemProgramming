 ...
 * ------------------------------------------------------------
 * PURPOSE:
 *   Creates and destroys the single anonymous shared-memory
 *   region (SharedData) used by every process in the system.
 *
 ...
 *     3. Is visible to all descendants without any extra
 *        attachment step.
 *   This makes it the simplest and most portable choice for
 *   a purely fork()-based system on Linux/POSIX.
 * ============================================================
 */
...
SharedData *shm_init(SystemConfig *config, WordInfo *words, int word_count) {
     ...
     *   sizeof(...)   – exact byte count needed.
     *   PROT_READ|WRITE – we need both read and write access.
     *   MAP_SHARED    – changes are visible to ALL processes that
     *                   inherit this mapping via fork().
     *   MAP_ANONYMOUS – not backed by a file; initialised to zero
     *                   by the kernel.
     *   -1, 0         – fd and offset; ignored for anonymous maps.
     */
    ...
    /*
     * Even though MAP_ANONYMOUS guarantees zero pages from the OS,
     * an explicit memset ensures correctness if this code were ever
     * ported to a non-anonymous mapping and prevents subtle bugs
     * from assumed zero-initialisation.
     */
    memset(data, 0, sizeof(SharedData));

    /* ── Copy system configuration ──────────────────────────── */
    /*
     * Every child process will read config fields directly from
     * shared memory, so we store a full copy here rather than
     * relying on the parent's local variable (which each child
     * has its own copy of after fork, but that copy cannot be
    ...
    /* ── Copy word list ─────────────────────────────────────── */
    /*
     * temp_words[] in main() is a local (stack) array.  We must
     * copy it into shared memory before forking so that all
     * child processes see the same word data.
     */
    ...
    /*
     * Each floor starts with zero active words and its initial
     * quota of letter-carriers (letter_carriers_per_floor).
     * floor_mutex and floor_cond are initialised later by sync_init().
     */
    ...
    /*
     * The delivery elevator starts at floor 0, idle, empty.
     * Its capacity is the value supplied via -d on the command line.
     */
 ...
 *
 * Calling munmap() while another process is still using the
 * region would cause undefined behaviour (access to unmapped
 * memory → SIGSEGV).
 * ============================================================ */
