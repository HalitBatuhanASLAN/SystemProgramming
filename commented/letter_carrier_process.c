 ...
 *
 * CONCURRENCY NOTES:
 *   - CharTask.claimed is set to 1 while holding word->word_mutex,
 *     so two carriers can never claim the same task.
 *   - The elevator request queue is protected by elev_mutex.
 *   - After enqueuing a request the carrier releases the mutex and
 *     waits on elev_cond (with a 50 ms timeout) until the elevator
 *     sets served==1 or removes the entry from the queue.
 *   - floor letter_carrier_count is updated under floor_mutex to
 *     keep the parent's monitoring accurate.
 * ============================================================
 */
...
/* ============================================================
 * request_delivery_elevator  (file-scope helper)
 * ------------------------------------------------------------
 * Adds a new entry to the delivery elevator's request queue so
 * that the elevator process will pick up this letter-carrier
 * (carrying one character) at from_floor and drop it off at
 * to_floor.
 *
 * After enqueuing:
 *   1. sem_post() wakes the elevator process (which sleeps on
 *      sem_timedwait when the queue is empty).
 *   2. pthread_cond_broadcast() wakes any other waiters on the
 *      elevator's condition variable.
 * ============================================================ */
static void request_delivery_elevator(SharedData *data,
                                       int from_floor, int to_floor,
                                       int carrier_id, int word_id,
                                       char character, int original_index) {
...
/* ============================================================
 * request_reposition_elevator  (file-scope helper)
 * ------------------------------------------------------------
 * Adds a new entry to the REPOSITION elevator's queue so that
 * an idle letter-carrier is moved to a random different floor.
 *
 * The target floor is chosen randomly here (not by the elevator)
 * so the carrier already knows its destination and can update
 * current_floor as soon as the ride completes.
 *
 * *out_to_floor receives the chosen destination floor so the
 * caller can wait for the correct request.
 * ============================================================ */
static void request_reposition_elevator(SharedData *data,
                                         int from_floor, int carrier_id,
                                         int *out_to_floor) {
    ElevatorState *elev = &data->reposition_elevator;
    int to_floor;

    /*
     * Pick a random target floor that is different from the
     * current floor.  The loop prevents staying in place when
...
/* ============================================================
 * deliver_char_to_sorting_area  (file-scope helper)
 * ------------------------------------------------------------
 * Places the transported character into the first available
 * (non-occupied, non-fixed) slot of the word's sorting_area[]
 * on the sorting floor.
 *
 * Assignment rule: characters are NOT placed directly into their
 * final correct index.  They go into the FIRST empty slot;
 * sorting processes then rearrange them.
 *
 ...
 * ============================================================ */
static void deliver_char_to_sorting_area(SharedData *data,
                                          int word_idx, char character,
     ...
     * (each word starts with all slots empty and chars arrive one
     * at a time), but releasing the mutex is still mandatory.
     */
    pthread_mutex_unlock(&word->word_mutex);
}

/* ============================================================
 * find_task_on_floor  (file-scope helper)
 * ------------------------------------------------------------
 * Searches for an unclaimed, undelivered CharTask among all
 * admitted, incomplete words whose arrival_floor matches
 * current_floor.
 *
 * Search order is randomised (random starting word, random
 * starting character) to reduce contention when multiple
 * letter-carriers are on the same floor.
 *
 * If a task is found, its claimed flag is set to 1 while
 * holding word->word_mutex, and the caller is given the
 * word index and char task index via output parameters.
 *
 * Returns 1 if a task was found and claimed, 0 otherwise.
 * ============================================================ */
 ...
 *   WORK MODE  – a CharTask was found; deliver it (same floor or
 *                via delivery elevator to a different floor).
 *   IDLE MODE  – no task available; use reposition elevator to
 *                move to a random floor with (hopefully) more work.
 *
 * The process exits when data->system_running becomes 0.
 * ============================================================ */
    ...
    while (data->system_running) {
        ...
        if (find_task_on_floor(data, current_floor, &word_idx, &char_idx)) {
            ...
            if (task->dest_floor == current_floor) {
                /*
                 * The character's destination is the current floor;
                 * no elevator is needed.  Place it directly.
                 */
                log_msg("Destination is same floor -> direct placement");

                deliver_char_to_sorting_area(data, word_idx,
                                              task->character,
                                              task->original_index,
                                              carrier_id);
                ...
                /*
                 * Cache the task fields before releasing word_mutex
                 * so we can safely reference them while waiting for
                 * the elevator (the pointer might become stale if
                 * the word were somehow relocated, though that does
                ...
                while (!arrived && data->system_running) {
                    ...
                    /*
                     * The elevator's clean_served_requests() may have
                     * already removed our entry (served==1 entries are
                     * deleted after processing).  If the request is no
                     * longer in the queue at all, delivery is done.
                     */
                    ...
                    if (!arrived && data->system_running) {
                        ...
                        pthread_cond_timedwait(&elev->elev_cond,
                                               &elev->elev_mutex, &ts);
                    }
                ...
                if (arrived) {
                    /*
                     * The carrier is now on the destination floor.
                     * Update the letter_carrier_count on both floors
                     * so the parent's monitoring stays accurate.
                     */
            ...
            if (data->config.num_floors > 1) {
                ...
                log_msg("Letter-carrier-process_%d requested reposition elevator "
                        "from floor %d", carrier_id, current_floor);

                int to_floor = current_floor;
                request_reposition_elevator(data, current_floor,
                                             carrier_id, &to_floor);
                 ...
                 * Sleep for 100 ms waiting for new work to appear
                 * (e.g., a word being admitted by a word-carrier).
                 */
                pthread_mutex_lock(&data->floors[current_floor].floor_mutex);
                if (data->system_running) {
                    ...
                    pthread_cond_timedwait(
                        &data->floors[current_floor].floor_cond,
