/*
 * ============================================================
 * FILE: sorting_process.h
 * ------------------------------------------------------------
 * PURPOSE:
 *   Public interface for the sorting process module.
 *
 * ROLE IN THE SYSTEM:
 *   Sorting processes run on a fixed floor and continuously
 *   scan words whose sorting_floor matches their assigned floor.
 *   As letter-carriers deliver characters into a word's
 *   sorting_area[], the sorting process rearranges them until
 *   the word is fully reconstructed (every fixed[i] == 1).
 *
 ...
 *
 * CONCURRENCY:
 *   Multiple sorting processes may exist on the same floor, but
 *   only ONE may operate on a given word at a time.  This is
 *   enforced by pthread_mutex_trylock(&word->word_mutex): a
 *   sorter that cannot immediately acquire the lock simply skips
 *   that word and checks it again on the next pass.
 * ============================================================
 */
 ...
 * Never changes its floor_id.
 *
 * Parameters:
 *   data      – pointer to the shared-memory region.
 *   floor_id  – the floor this sorter is permanently assigned to.
 *   sorter_id – unique numeric ID for logging and statistics.
 */
