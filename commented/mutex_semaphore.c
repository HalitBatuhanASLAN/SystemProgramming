 ...
 * ------------------------------------------------------------
 * PURPOSE:
 *   Initialises and destroys all synchronisation primitives
 *   stored inside the shared-memory region (SharedData).
 *
 * KEY CONCEPT – PROCESS-SHARED PRIMITIVES:
 *   Normally pthread_mutex_t and pthread_cond_t only synchronise
 *   threads within a single process.  To make them work across
 *   fork()-ed processes that share the same mmap() region, each
 *   primitive must be created with the PTHREAD_PROCESS_SHARED
 *   attribute.  The two static helper functions below handle
 *   this boilerplate for mutexes and condition variables.
 *
 *   POSIX semaphores are made process-shared by passing pshared=1
 *   to sem_init().
 * ============================================================
 */
 ...
 * ------------------------------------------------------------
 * Creates a single pthread_mutex_t that can be used by multiple
 * independent processes (not just threads).
 *
 * Steps:
 *   1. Initialise a pthread_mutexattr_t.
 *   2. Set PTHREAD_PROCESS_SHARED so the kernel allows cross-
 *      process locking.
 *   3. Initialise the mutex with those attributes.
 *   4. Destroy the (temporary) attribute object.
 *
 ...
 * ------------------------------------------------------------
 * Creates a single pthread_cond_t that can be signalled and
 * waited on across multiple independent processes.
 *
 * The pattern is identical to init_shared_mutex: create an
 * attribute object, set PTHREAD_PROCESS_SHARED, initialise
 * the cond, destroy the attribute.
 *
 * Returns 0 on success, -1 on any failure.
 * ============================================================ */
 ...
 * Called once by the parent process after shm_init() and before
 * any child processes are forked.
 *
 * Initialisation order (matches the declaration order in
 * common.h for clarity, but order does not matter for correctness
 * because no process uses the primitives yet):
 *   1. Per-word mutexes          (one per WordInfo)
 *   2. Per-floor mutex + cond    (one pair per Floor)
 *   3. Delivery elevator:        mutex, cond, semaphore
 *   4. Reposition elevator:      mutex, cond, semaphore
 *   5. Global mutexes:           round_robin, stats, state
 *   6. Global cond:              state_cond
 *   7. Children list mutex
 *
 * Returns 0 on full success, -1 on first failure (partial init
 * state; caller should abort).
 * ============================================================ */
int sync_init(SharedData *data) {

    /* ── 1. Per-word mutexes ─────────────────────────────────── */
    /*
     * Each word has its own mutex so that multiple sorting
     * processes on the same floor can work on DIFFERENT words
     * simultaneously, but only ONE sorter can modify a given
     * word at a time.
     */
    ...
    /*
     * floor_mutex protects active_word_count and
     * letter_carrier_count.  floor_cond is broadcast whenever
     * one of those counts changes so that processes waiting for
     * capacity or carriers wake up promptly.
     */
    ...
    /*
     * elev_mutex protects the request queue, current floor,
     * direction, and load.
     * elev_cond is used by letter-carrier processes to wait for
     * their delivery request to reach served==1.
     * request_sem is posted by any carrier that enqueues a new
     * request; the elevator process blocks on sem_timedwait()
    ...
    /*
     * Identical structure to the delivery elevator but operates
     * independently on a separate request queue.
     */
    ...
    /*
     * Protects round_robin_index and the claimed flag during
     * atomic word selection by word-carrier processes.
     * Without this lock two carriers could claim the same word.
     */
    ...
    /* ── 6. Statistics mutex ────────────────────────────────── */
    /*
     * Protects all counters in SharedData (total_retries,
     * total_chars_transported, per-process arrays, …) so
     * concurrent increments do not produce race conditions.
     */
    ...
    /*
     * state_mutex / state_cond form a general-purpose
     * notification channel: whenever any significant state changes
     * (word admitted, character delivered, word completed) a
     * broadcast wakes any process that was sleeping here.
     */
    ...
    /*
     * Protects child_pids[] and num_children so that the parent
     * can safely register new PIDs from within the forking loop
 ...
 * sync_destroy
 * ------------------------------------------------------------
 * Destroys every synchronisation primitive in the reverse order
 * they were created.  Must be called after all child processes
 * have exited so no primitive is in use during destruction.
 *
 * Failing to destroy primitives before munmap() would leave
 * kernel resources (futexes, semaphore objects) leaked.
 * ============================================================ */
