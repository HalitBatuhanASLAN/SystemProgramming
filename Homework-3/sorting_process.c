 ...
 * ALGORITHM OVERVIEW:
 * Each iteration of the outer while loop scans every word on the assigned floor.  For each word it tries to acquire word_mutex (non-blocking with trylock) and, if successful, performs one pass of the sorting algorithm.
 * NO BUSY-WAITING:
 * If a complete scan of the floor's words produced no progress (did_work == 0), the process sleeps on floor_cond with a 100 ms timeout until a letter-carrier delivers a new character (which broadcasts on floor_cond).
 */
...
void sorting_process_run(SharedData *data, int floor_id, int sorter_id)
{
    while(data->system_running)
    {
        ...
        for(int i = 0;i<data->total_words && data->system_running;i++)
        {
            ...
            if(word->sorting_floor != floor_id)
                continue;
            ...
            /*
             * Use trylock (non-blocking) so we don't stall this sorter waiting for a word that another sorter is currently processing.  We simply move on to the next word and try again later.
             */
            if(pthread_mutex_trylock(&word->word_mutex) != 0)
                continue;

            /*
             * Re-check completed after acquiring the lock because another sorter may have just finished this word between our initial check and the lock acquisition.
             */
            if(word->completed)
            {
            ...
            /*
             * If no character has arrived yet there is nothing to sort; skip and wait for letter-carriers.
             */
            if(!has_occupied)
            {
                pthread_mutex_unlock(&word->word_mutex);
            ...
            for(int j = 0;j<word->word_len;j++)
            {
                ...
                /*
                 * Find the first unfixed index t where word[t] equals current_char (i.e., where this character belongs).We stop at the first match to avoid unnecessary moves.
                 */
                int target_pos = -1;
                ...
                /* Edge case: target is the same slot. */
                if(target_pos == j)
                {
                    if(word->sorting_area[j] == word->word[j])
                    {
            ...
            if(all_fixed)
            {
                ...
                /*
                 * Decrement active_word_count on the arrival floor.This frees a slot so word-carriers waiting for capacity can admit another word.
                 */
                ...
                /*
                 * Also decrement on the sorting floor if it is different from the arrival floor.
                 */
                ...
                /* Broadcast on the global state cond. */
        ...
        if(!did_work)
        {
            ...
            if(data->system_running)
            {
                ...
                pthread_cond_timedwait(&data->floors[floor_id].floor_cond,&data->floors[floor_id].floor_mutex,&ts);
            }
