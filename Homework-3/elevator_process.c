...
/* 
 * Removes all requests with served == 1 from the elevator's queue by compacting the array in-place (shifting elements with served != 1 toward index 0).
 *
 * This is called at the start of each elevator cycle so the queue only ever contains pending or in-transit requests.
 *
...
/*
 * Returns 1 if the queue contains at least one request that has not yet been fully delivered (served == 0 or served == 2).
...
/* Inspects the first unclaimed (served == 0) request and returns the direction the elevator should take to reach it from the current floor:
 * DIR_UP   – the request's from_floor is above current.
 * DIR_DOWN – the request's from_floor is below current.
 * DIR_IDLE – request is on the current floor; use to_floor to decide (character needs to go up or down).
 *
 * Must be called with elev_mutex held.*/
...
void delivery_elevator_run(SharedData *data)
{
    ...
    while(data->system_running)
    {
        ...
        /*
         * sem_timedwait blocks until a carrier calls sem_post() after enqueuing a request, OR until the 50 ms timeout expires (whichever comes first).  The timeout prevents the elevator from freezing if a sem_post is missed.
         */
        struct timespec ts;
        ...
        /*
         * If the elevator was idle, choose a direction based on the oldest pending request's location.
         */
        ...
        for(int i = 0;i<elev->queue_size;i++)
        {
                ...
                elev->queue[i].to_floor == elev->current_floor)
            {
                ...
                /*
                 * Release the lock before calling log_msg() to avoid holding it during printf (which can be slow and would block any carrier trying to enqueue a new request).
                 */
                pthread_mutex_unlock(&elev->elev_mutex);
                ...
                /*
                 * Broadcast on elev_cond: the waiting carrier process is polling for served==1 on elev_cond; wake it now.
                 */
        ...
        /*
         * Search the queue for any request (waiting or in transit) that lies beyond the current floor in our travel direction.
        ...
        for(int i = 0;i<elev->queue_size;i++)
        {
            ...
            /*
             * For a waiting request the relevant floor is from_floor (where we need to pick them up).
             * For an in-transit request it is to_floor (where we need to drop them off).
             */
        ...
        /*
         * If no request lies further in the current direction AND the elevator is empty, reconsider direction.
         */
...
/* REPOSITION ELEVATOR
 * Identical movement policy to the delivery elevator; the only differences are:
 * - It operates on data->reposition_elevator (separate queue).
 * - It carries letter-CARRIERS (not characters), so log messages show carrier_id only (no word_id / character fields).
 * - It updates data->reposition_elevator_ops instead of delivery_elevator_ops.
*/
