...
/*
 Prints a concise usage summary to stderr so the user knows which flags are required and what they control.
...
int parse_args(int argc, char *argv[], SystemConfig *config)
{
    ...
    /*
     * Presence flags: each is set to 1 when the corresponding command-line option is seen.  After the loop we verify that ALL of them are 1 (every parameter is mandatory).
     */
    ...
    while((opt = getopt(argc, argv, "f:w:l:s:c:d:r:i:o:")) != -1)
    {
        switch(opt)
        {
            ...
            /*
             * -i <input_file> strncpy + explicit NUL ensures we cannot overflow the fixed-size buffer even with a very long path.
             */
            case 'i':
    ...
    /*
     * access(path, R_OK) returns 0 if the calling process can read the file right now.  We check this early so the user gets a clear error message instead of a cryptic fopen() failure later inside read_input_file().
     */
