 ...
 *
 * ROLE IN THE SYSTEM:
 * Letter-carrier processes are the "couriers" of the system.They move individual characters from the word's arrival floor to its sorting floor, using the delivery elevator whenever the source and destination floors differ.
 *
 * Unlike word-carrier processes (which never leave their assigned floor), a letter-carrier is MOBILE: after delivering a character it stays on the destination floor and searches for new work there.  If no work is available on the current floor it uses the REPOSITION elevator to move to a random floor where work might be waiting.
 *
...
#include "common.h"   /* Brings in SharedData and standard headers */
