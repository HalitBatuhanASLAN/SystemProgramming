/*
 * PURPOSE:
 * Entry point and coordinator (parent process) for the multi-process word transportation and concurrent sorting system.
 *
 ...
 * Declared global so the SIGINT signal handler can reach it without needing it to be passed as a parameter (signal handlers receive only the signal number).
 */
static SharedData *g_shared_data = NULL;

/* Signal handler for SIGINT (Ctrl+C) and SIGTERM.
 * Sets system_running = 0 in shared memory.  Because the flag is declared volatile in SharedData, all processes that are currently spinning on "while (data->system_running)" will see the change on their next loop iteration and exit their main loop.
 * Declared with (void)sig to suppress the "unused parameter" compiler warning while keeping the correct sa_handler signature.
 * */
...
/* Appends a child PID to the parent's tracking list so that cleanup_children() can later send signals and collect them with waitpid().
 *
 * Protected by children_mutex because, in principle, multiple threads could register children concurrently (though in this single-threaded parent this is for safety/correctness only).
*/
...
/* Sends SIGTERM to every child, waits briefly, then sends SIGKILL to any that are still alive, and finally collects all of them with waitpid() to prevent zombie processes.
 * Two-phase shutdown:
 * Phase 1 – SIGTERM: gives children a chance to exit their main loop cleanly (they check system_running).
 * Phase 2 – SIGKILL (after 100 ms): forcefully terminates any child that did not respond to SIGTERM.
...
/*Prints the final system statistics to stdout after all words have been sorted (or after a SIGINT interruption).
*/
...
/* Returns 1 if every word in shared memory has completed == 1,meaning the sorting process has fully reconstructed it.Returns 0 if any word is still pending.
*/
static int check_all_completed(SharedData *data)
{
    for(int i = 0;i<data->total_words;i++)
    {
        if(!data->words[i].completed)
            return 0;
    }
    return 1;
}
...
int main(int argc, char *argv[])
{
    ...
    /*
     * Validate that each word's sorting_floor is within the configured range [0, num_floors - 1].  This check must happen BEFORE shared memory is allocated so failures are reported cleanly without resource leaks.
     */
    ...
    /*
     * sigaction() is preferred over signal() because its behaviour is well-defined by POSIX (signal() semantics vary by OS).
     */
    struct sigaction sa;
    ...
    /*
     * SIGTERM uses the same handler so that both Ctrl+C and a programmatic kill() result in a clean shutdown.
     */
     ...
     *
     * global_carrier_id is incremented for every child so every process has a unique identifier used in log messages and per-process statistics arrays.
     */
    for(int floor = 0;floor<config.num_floors;floor++)
    {
        ...
        for(int w = 0;w<config.word_carriers_per_floor;w++)
        {
            ...
            if(pid == 0)
            {
                 ...
                 * Re-seed rand() with a different value from the parent to ensure each child makes independent random choices.
                 */
                srand(time(NULL) ^ getpid());
     ...
     * When all words are sorted it sets system_running = 0 to signal every child to exit its loop.
     */
    ...
    if(check_all_completed(data))
    {
        /*Normal completion: write the sorted output and print the full summary.*/
    ...
    else
    {
         ...
         * Count how many words did finish and print a partial summary so the user knows what happened.
         */
        int completed = 0;
        for(int i = 0;i<data->total_words;i++)
        {
    ...
    /*
     * Destroy synchronisation primitives first (before the shared memory region disappears) then unmap the region.
     */
    sync_destroy(data);
    shm_destroy(data);
