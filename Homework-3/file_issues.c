 ...
 *
 * DESIGN NOTES:
 * - read_input_file() performs strict line-by-line validation so malformed input is caught before any process is forked.
 * - write_output_file() works on a temporary copy so the shared-memory words[] array is never reordered.
 * - Both functions use only stdio (fopen/fgets/fprintf) which is simpler and more portable than raw syscalls for file I/O.
 */
...
/*
 * Comparison function used by write_output_file() to sort the output array.
 *
 * Primary key:   sorting_floor  (ascending)
 * Secondary key: word_id         (ascending, breaks ties)
 *
 * This matches the output format required by the assignment: rows ordered first by which floor they sort on, then by the word's unique numeric identifier.*/
 ...
 *
 * After validation, each WordInfo is zero-filled and then populated including its CharTask array (one entry per letter).*/
int read_input_file(const char *path, WordInfo *words, int max_words)
{
    ...
    while(fgets(line, sizeof(line), fp) != NULL)
    {
        ...
        /*
         * If the line did not end with '\n' and we are NOT at EOF, the line is too long for our buffer (or the file uses non-standard line endings).
         */
        if(line[len - 1] != '\n' && !feof(fp))
        {
        ...
        /*
         * If the last line of the file has no trailing newline (which is valid on some systems), synthesise one so the rest of the parsing logic can assume '\n' always exists.
         */
        if(line[len - 1] != '\n' && feof(fp))
        {
        ...
        if(id_len   == 0 || id_len   >= sizeof(id_buf)   || word_len == 0 || word_len >= sizeof(word_buf)  || sf_len   == 0 || sf_len   >= sizeof(sf_buf))
        {
            fprintf(stderr,"Error: Invalid input format at line %d\n", line_no);
            fclose(fp);
            return -1;
        ...
        /*
         * strtol() with endptr lets us detect trailing garbage (e.g., "101abc") that atoi() would silently accept.
         */
        errno = 0;
        ...
        /*
         * Each character of the word becomes an independent task that a letter-carrier process will claim and deliver. src_floor is set to -1 here; it will be updated to the actual arrival_floor when a word-carrier admits the word.
         */
...
/*
 * Uses a temporary malloc'd copy of the words[] array so the original shared-memory data is never reordered (other processes might still be referencing it during shutdown).
...
int write_output_file(const char *path, WordInfo *words, int count)
{
    ...
    /*
     * qsort() is not stable, but the two-key comparator in compare_words() provides a total order so the result is deterministic regardless of the initial array order.
     */
