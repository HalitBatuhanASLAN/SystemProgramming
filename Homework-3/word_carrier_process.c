 ...
 * All word-carriers share a single round_robin_index counter protected by round_robin_mutex.  This guarantees that the same word is never claimed by two carriers simultaneously and that all words are visited fairly.
 *
 * All-or-nothing admission
 * A word may be admitted ONLY if BOTH its arrival floor AND its sorting floor have capacity.  To check both atomically we hold both floor locks at the same time.
 *
 * Deadlock prevention
 * When two floor locks must be held together we always acquire them in ascending floor-number order (lower-numbered floor first).  This breaks any potential circular wait.
 *
 * No busy-waiting
 * If admission fails or no unclaimed word is found, the process sleeps on state_cond with a 100 ms timeout instead of spinning.  It wakes up when any state change occurs (e.g., a word completes and frees floor capacity).
 */
...
void word_carrier_run(SharedData *data, int floor_id, int carrier_id)
{
    while(data->system_running)
    {
        ...
        {
            /*
             * All words are in the system.  Signal the other processes and let this carrier exit gracefully.
             */
        ...
        /*
         * Scan words[] starting at round_robin_index, wrapping around, until we find a word that is neither claimed nor already admitted.
         */
        for(int attempt = 0;attempt<data->total_words;attempt++)
        {
            ...
            if(!data->words[idx].claimed && !data->words[idx].admitted)
            {
                ...
                /*
                 * Mark as claimed WHILE holding the round-robin lock so no other carrier can pick this word in the same scan cycle.
                 */
        ...
        {
            /*
             * Sleep until a state change (word completion frees a floor slot, or another word gets admitted). The 100 ms timeout prevents permanent blocking if a broadcast is missed for any reason.
             */
        ...
        if(arrival_ok && sorting_ok)
        {
            ...
            /*
             * Set src_floor in every CharTask now that we know where the word physically arrives.
             */
            ...
            /*
             * Broadcast on the arrival floor so that letter-carrier processes sleeping there wake up and start picking up characters immediately.
             */
        ...
        else
        {
            ...
            /*
             * Reset the claimed flag so this word is available again for the next scan by any word-carrier.
             */
            pthread_mutex_lock(&data->round_robin_mutex);
            word->claimed = 0;
            pthread_mutex_unlock(&data->round_robin_mutex);
            ...
            /*
             * Sleep briefly before retrying.  Waiting on state_cond means we wake up as soon as any word completes
             */
            pthread_mutex_lock(&data->state_mutex);
